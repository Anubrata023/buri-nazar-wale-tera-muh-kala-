...
def seed_production_database():
    ...
    # Map ward names to ward_type from the main wards dataset (which includes ward_type column)
    ward_type_mapping = {}
    if os.path.exists("datasets/processed/lucknow_wards_clean.csv"):
        try:
            original_wards = pd.read_csv("datasets/processed/lucknow_wards_clean.csv")
            for _, row in original_wards.iterrows():
                if "ward_type" in row:
                    ward_type_mapping[str(row["ward_name"]).strip()] = str(row["ward_type"]).strip()
        except Exception as e:
            print(f"⚠️ Warning loading ward types: {e}")
    ...
    for _, row in merged_df.iterrows():
        ...
        try:
            ...
            print(f"Upserted ward: {ward_name} ({ward_type})")
        except Exception as e:
...
919800000005,Chinhat,"Mera handpump bhi kharab hai, 47 log ne report kiya - pani nahi aa raha",Water,8,Same handpump broken - 47 citizens reported - merged issue,Wahi handpump - 47 log ne report kiya - merged issue,26.86,80.94"""
    ...
    for _, row in complaints_df.iterrows():
        ...
        try:
            ...
            print(f"Inserted demo complaint for: {ward_name}")
        except Exception as e:
